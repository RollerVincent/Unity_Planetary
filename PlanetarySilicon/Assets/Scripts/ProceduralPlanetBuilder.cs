...
[ExecuteInEditMode]
public class ProceduralPlanetBuilder : MonoBehaviour
{
    ...
    public GameObject CreateInstance(Vector3 position, SystemManager parentSystem){
        ...
        float radiusVariance = GetComponent<PlanetType>().radiusVariance;

        float radiusOffset = Random.Range(-radiusVariance, radiusVariance);

        Radius = Radius + Radius * radiusOffset;

        float factor = Radius / tmpRadius;


        // set seed to system star index 
        GameObject newObj = AdobtMesh();

        PlanetInstance planetInstance = newObj.AddComponent<PlanetInstance>();
        planetInstance.planetType = GetComponent<PlanetType>();
        planetInstance.radius = Radius;



        newObj.name = gameObject.name.Split("Type")[1] + " Instance";
            ...
            if(child.name != "Planar Surface" && child.name != "Marching Surface" && child.name != "Resource Manager"){
                if(!child.name.Contains("Decoration")){
                    ...
                    if(child.name.Contains("Water")){
                        ...
                        newChild.transform.localScale = newChild.transform.localScale * factor;
                    }
       ...
       /* foreach(Decorator decorator in newObj.GetComponents<Decorator>()){
        ...
        newObj.AddComponent<GravityObject>().mass = planetInstance.planetType.mass;
