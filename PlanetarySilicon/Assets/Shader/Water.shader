	...
	SubShader {
		...
		Pass {
			...
			float sampleTexture3D(float3 position, sampler2D tex, float detail) {
				...
				position = ((normalize(position) + float3(1.25,3.50,5.75))/detail) * _Radius*0.01;
			...
			v2f vert (appdata_base v) {
				...
				o.worldPos = unity_ObjectToWorld._m03_m13_m23 + v.vertex * (_Radius + (offset1*_NoiseImpact1 * pow(offset2,_NoiseImpact2))+offset3*_NoiseImpact3);
