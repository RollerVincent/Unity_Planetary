Shader "Custom/PlanetaryFog2"
{
    Properties {
        ...
        _DiffractionDensity("DiffractionDensity", Range(0,10)) = 1
	...
	SubShader
	{
		...
		Pass
		{
            ...
            //    o.worldPos += float4((o.uv.y*dy*_Radius), 0);
				...
				o.worldPos.w = max(0,dot(planetPos, _WorldSpaceLightPos0.xyz)*-1);
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                float globalIntensity = 1 - max(0,dot(normalize(i.worldPos-_Center), _WorldSpaceLightPos0.xyz)*-1);
                float globalIntensity2 = (dot(normalize(i.worldPos-_Center), _WorldSpaceLightPos0.xyz)+1)/2;

                //float globalIntensity = 1-i.worldPos.w;
                float centerUv = 1-saturate((pow((i.uv.x-0.5)*2,2) + pow((i.uv.y-0.5)*2,2)));
                clip(centerUv-0.001);

                float diffractionFactor = (1-globalIntensity2)*1 + (centerUv*_DiffractionDensity)*(1-1);
                ...
                //return globalIntensity*centerUv;

                

                float4 lightDiffraction = _Diffraction * diffractionFactor;

                //return _Diffraction;

                float4 lightColor = _Color - lightDiffraction;

                lightColor.a = centerUv;

                return lightColor * saturate(globalIntensity);

                return lightColor*globalIntensity*centerUv;
                ...
                fogColor.a= min(centerUv, 1)*depthAlpha;
